export const items = [
  { 
    name: "Bank Central Asia Tbk.",
    sector: "Financials",
    all: 90.79,
    anti_coruption: 84.38,
    environment: 85.71,
    social: 100.00
  },
  { 
    name: "Medco Energi Internasional Tbk",
    sector: "Energy",
    all: 84.21,
    anti_coruption: 78.13,
    environment: 85.71,
    social: 90.00
  },
  { 
    name: "Astra International Tbk",
    sector: "Industrials",
    all: 84.21,
    anti_coruption: 71.88,
    environment: 100.00,
    social: 90.00
  },
  { 
    name: "Bank Mandiri (Persero) Tbk.",
    sector: "Financials",
    all: 82.89,
    anti_coruption: 71.88,
    environment: 85.71,
    social: 93.33
  },
  { 
    name: "PT Bank Rakyat Indonesia (Persero) Tbk",
    sector: "Financials",
    all: 82.89,
    anti_coruption: 71.88,
    environment: 85.71,
    social: 93.33
  },
  { 
    name: "Bumi Resources Tbk",
    sector: "Energy",
    all: 81.58,
    anti_coruption: 78.13,
    environment: 85.71,
    social: 83.33
  },
  { 
    name: "PT Bank CIMB Niaga Tbk",
    sector: "Financials",
    all: 80.26,
    anti_coruption: 78.13,
    environment: 78.57,
    social: 83.33
  },
  { 
    name: "PT XL Axiata Tbk",
    sector: "Infrastructures",
    all: 78.95,
    anti_coruption: 75.00,
    environment: 71.43,
    social: 86.67
  },
  { 
    name: "AKR Corporindo Tbk.",
    sector: "Energy",
    all: 78.95,
    anti_coruption: 71.88,
    environment: 85.71,
    social: 83.33
  },
  { 
    name: "United Tractors Tbk",
    sector: "Industrials",
    all: 78.95,
    anti_coruption: 75.00,
    environment: 78.57,
    social: 83.33
  },
  { 
    name: "Bukit Asam Tbk",
    sector: "Energy",
    all: 77.63,
    anti_coruption: 65.63,
    environment: 85.71,
    social: 86.67
  },
  { 
    name: "Elang Mahkota Teknologi Tbk",
    sector: "Technology",
    all: 76.32,
    anti_coruption: 71.88,
    environment: 50.00,
    social: 93.33
  },
  { 
    name: "Perusahaan Gas Negara Tbk",
    sector: "Energy",
    all: 75.00,
    anti_coruption: 71.88,
    environment: 71.43,
    social: 80.00
  },
  { 
    name: "Garuda Indonesia (Persero) Tbk",
    sector: "Transportation & Logistic",
    all: 73.68,
    anti_coruption: 65.63,
    environment: 85.71,
    social: 76.67
  },
  { 
    name: "Perusahaan Listrik Negara (persero)",
    sector: "Infrastructures",
    all: 73.68,
    anti_coruption: 65.63,
    environment: 71.43,
    social: 83.33
  },
  { 
    name: "Pertamina (persero)",
    sector: "Infrastructures",
    all: 72.37,
    anti_coruption: 62.50,
    environment: 78.57,
    social: 80.00
  },
  { 
    name: "PT Sinas Mas Agro Resources and Technology Tbk",
    sector: "Consumer Non-Cyclicals",
    all: 72.37,
    anti_coruption: 62.50,
    environment: 71.43,
    social: 83.33
  },
  { 
    name: "Erajaya Swasembada Tbk",
    sector: "Consumer Cyclicals",
    all: 72.37,
    anti_coruption: 71.88,
    environment: 71.43,
    social: 73.33
  },
  { 
    name: "ABM Investama Tbk",
    sector: "Industrials",
    all: 72.37,
    anti_coruption: 56.25,
    environment: 85.71,
    social: 83.33
  },
  { 
    name: "Indocement Tunggal Prakarsa Tbk",
    sector: "Basic Materials",
    all: 72.37,
    anti_coruption: 62.50,
    environment: 85.71,
    social: 76.67
  },
  { 
    name: "PP (Persero) Tbk",
    sector: "Infrastructures",
    all: 71.05,
    anti_coruption: 65.63,
    environment: 57.14,
    social: 83.33
  },
  { 
    name: "GoTo Gojek Tokopedia Tbk.",
    sector: "Technology",
    all: 71.05,
    anti_coruption: 75.00,
    environment: 85.71,
    social: 60.00
  },
  { 
    name: "Astra Agro Lestari Tbk",
    sector: "Consumer Non-Cyclicals",
    all: 71.05,
    anti_coruption: 62.50,
    environment: 71.43,
    social: 80.00
  },
  { 
    name: "Unilever Indonesia Tbk.",
    sector: "Consumer Non-Cyclicals",
    all: 71.05,
    anti_coruption: 71.88,
    environment: 64.29,
    social: 73.33
  },
  { 
    name: "Delta Dunia Makmur Tbk",
    sector: "Energy",
    all: 69.74,
    anti_coruption: 71.88,
    environment: 57.14,
    social: 73.33
  },
  { 
    name: "Sarana Menara Nusantara Tbk",
    sector: "Infrastructures",
    all: 69.74,
    anti_coruption: 62.50,
    environment: 78.57,
    social: 73.33
  },
  { 
    name: "Lippo Karawaci Tbk",
    sector: "Properties & Real Estate",
    all: 69.74,
    anti_coruption: 71.88,
    environment: 64.29,
    social: 70.00
  },
  { 
    name: "Krakatau Steel (Persero) Tbk",
    sector: "Basic Materials",
    all: 69.74,
    anti_coruption: 71.88,
    environment: 78.57,
    social: 63.33
  },
  { 
    name: "Mayora Indah Tbk",
    sector: "Consumer Non-Cyclicals",
    all: 68.42,
    anti_coruption: 65.63,
    environment: 42.86,
    social: 83.33
  },
  { 
    name: "PT Bank Danamon Indonesia Tbk",
    sector: "Financials",
    all: 68.42,
    anti_coruption: 68.75,
    environment: 64.29,
    social: 70.00
  },
  { 
    name: "Surya Esa Perkasa Tbk",
    sector: "Basic Materials",
    all: 68.42,
    anti_coruption: 53.13,
    environment: 85.71,
    social: 76.67
  },
  { 
    name: "Dian Swastatika Sentosa Tbk",
    sector: "Energy",
    all: 68.42,
    anti_coruption: 62.50,
    environment: 78.57,
    social: 70.00
  },
  { 
    name: "Jasa Marga (Persero) Tbk",
    sector: "Infrastructures",
    all: 68.42,
    anti_coruption: 50.00,
    environment: 71.43,
    social: 86.67
  },
  { 
    name: "PT Wijaya Karya (Persero) Tbk",
    sector: "Infrastructures",
    all: 68.42,
    anti_coruption: 50.00,
    environment: 78.57,
    social: 83.33
  },
  { 
    name: "Merdeka Copper Gold Tbk.",
    sector: "Basic Materials",
    all: 68.42,
    anti_coruption: 68.75,
    environment: 64.29,
    social: 70.00
  },
  { 
    name: "Adaro Minerals Indonesia Tbk",
    sector: "Energy",
    all: 68.42,
    anti_coruption: 62.50,
    environment: 78.57,
    social: 70.00
  },
  { 
    name: "HM Sampoerna Tbk",
    sector: "Consumer Non-Cyclicals",
    all: 68.42,
    anti_coruption: 65.63,
    environment: 57.14,
    social: 76.67
  },
  { 
    name: "Tunas Baru Lampung Tbk",
    sector: "Consumer Non-Cyclicals",
    all: 67.11,
    anti_coruption: 68.75,
    environment: 71.43,
    social: 63.33
  },
  { 
    name: "Bank Permata Tbk",
    sector: "Financials",
    all: 67.11,
    anti_coruption: 78.13,
    environment: 42.86,
    social: 66.67
  },
  { 
    name: "Indah Kiat Pulp & Paper Tbk",
    sector: "Basic Materials",
    all: 67.11,
    anti_coruption: 68.75,
    environment: 35.71,
    social: 80.00
  },
  { 
    name: "Bank Mega Tbk",
    sector: "Financials",
    all: 65.79,
    anti_coruption: 62.50,
    environment: 57.14,
    social: 73.33
  },
  { 
    name: "Bank Pembangunan Daerah Jawa Barat dan Banten Tbk",
    sector: "Financials",
    all: 65.79,
    anti_coruption: 68.75,
    environment: 50.00,
    social: 70.00
  },
  { 
    name: "Chandra Asri Petrochemical Tbk",
    sector: "Basic Materials",
    all: 65.79,
    anti_coruption: 59.38,
    environment: 64.29,
    social: 73.33
  },
  { 
    name: "Harum Energy Tbk",
    sector: "Energy",
    all: 65.79,
    anti_coruption: 53.13,
    environment: 85.71,
    social: 70.00
  },
  { 
    name: "Barito Pacific Tbk",
    sector: "Basic Materials",
    all: 65.79,
    anti_coruption: 59.38,
    environment: 71.43,
    social: 70.00
  },
  { 
    name: "PT Saratoga Investama Sedaya Tbk.",
    sector: "Financials",
    all: 65.79,
    anti_coruption: 50.00,
    environment: 64.29,
    social: 83.33
  },
  { 
    name: "Pelabuhan Indonesia (PELINDO) (persero)",
    sector: "Transportation & Logistic",
    all: 64.47,
    anti_coruption: 62.50,
    environment: 64.29,
    social: 66.67
  },
  { 
    name: "PT Adhi Karya (Persero) Tbk.",
    sector: "Infrastructures",
    all: 64.47,
    anti_coruption: 65.63,
    environment: 50.00,
    social: 70.00
  },
  { 
    name: "Pabrik Kertas Tjiwi Kimia Tbk",
    sector: "Basic Materials",
    all: 64.47,
    anti_coruption: 62.50,
    environment: 50.00,
    social: 73.33
  },
  { 
    name: "Sumber Alfaria Trijaya Tbk.",
    sector: "Consumer Non-Cyclicals",
    all: 63.16,
    anti_coruption: 53.13,
    environment: 78.57,
    social: 66.67
  },
  { 
    name: "Pratama Abadi Nusa Industri Tbk",
    sector: "Consumer Non-Cyclicals",
    all: 63.16,
    anti_coruption: 50.00,
    environment: 50.00,
    social: 83.33
  },
  { 
    name: "Ciputra Development Tbk",
    sector: "Properties & Real Estate",
    all: 63.16,
    anti_coruption: 59.38,
    environment: 78.57,
    social: 60.00
  },
  { 
    name: "Gunung Raja Paksi Tbk",
    sector: "Basic Materials",
    all: 63.16,
    anti_coruption: 65.63,
    environment: 57.14,
    social: 63.33
  },
  { 
    name: "Capital Financial Indonesia Tbk",
    sector: "Financials",
    all: 63.16,
    anti_coruption: 62.50,
    environment: 64.29,
    social: 63.33
  },
  { 
    name: "Smartfren Telecom Tbk",
    sector: "Infrastructures",
    all: 61.84,
    anti_coruption: 53.13,
    environment: 85.71,
    social: 60.00
  },
  { 
    name: "Indo Tambangraya Megah Tbk",
    sector: "Energy",
    all: 61.84,
    anti_coruption: 65.63,
    environment: 78.57,
    social: 50.00
  },
  { 
    name: "Aneka Tambang Tbk.",
    sector: "Basic Materials",
    all: 60.53,
    anti_coruption: 46.88,
    environment: 71.43,
    social: 70.00
  },
  { 
    name: "Kereta Api Indonesia (persero)",
    sector: "Transportation & Logistic",
    all: 60.53,
    anti_coruption: 71.88,
    environment: 57.14,
    social: 50.00
  },
  { 
    name: "PT Tower Bersama Infrastructure Tbk",
    sector: "Infrastructures",
    all: 60.53,
    anti_coruption: 78.13,
    environment: 64.29,
    social: 40.00
  },
  { 
    name: "Bayan Resources Tbk",
    sector: "Energy",
    all: 60.53,
    anti_coruption: 53.13,
    environment: 42.86,
    social: 76.67
  },
  { 
    name: "Adaro Energy Indonesia Tbk",
    sector: "Energy",
    all: 60.53,
    anti_coruption: 53.13,
    environment: 64.29,
    social: 66.67
  },
  { 
    name: "PT Bank Mayapada Internasional Tbk",
    sector: "Financials",
    all: 59.21,
    anti_coruption: 62.50,
    environment: 42.86,
    social: 63.33
  },
  { 
    name: "PT Bank Maybank Indonesia Tbk",
    sector: "Financials",
    all: 59.21,
    anti_coruption: 53.13,
    environment: 64.29,
    social: 63.33
  },
  { 
    name: "Gajah Tunggal Tbk",
    sector: "Consumer Cyclicals",
    all: 59.21,
    anti_coruption: 53.13,
    environment: 64.29,
    social: 63.33
  },
  { 
    name: "PT MNC Investama Tbk.",
    sector: "Industrials",
    all: 59.21,
    anti_coruption: 59.38,
    environment: 42.86,
    social: 66.67
  },
  { 
    name: "PT Bank OCBC NISP Tbk",
    sector: "Financials",
    all: 57.89,
    anti_coruption: 68.75,
    environment: 64.29,
    social: 43.33
  },
  { 
    name: "Astra Otoparts Tbk",
    sector: "Consumer Cyclicals",
    all: 57.89,
    anti_coruption: 46.88,
    environment: 64.29,
    social: 66.67
  },
  { 
    name: "Telkom Indonesia (Persero) Tbk.",
    sector: "Infrastructures",
    all: 57.89,
    anti_coruption: 53.13,
    environment: 50.00,
    social: 66.67
  },
  { 
    name: "Indika Energy Tbk",
    sector: "Energy",
    all: 57.89,
    anti_coruption: 56.25,
    environment: 35.71,
    social: 70.00
  },
  { 
    name: "Mining Industry Indonesia (MIND ID)",
    sector: "Basic Materials",
    all: 57.89,
    anti_coruption: 56.25,
    environment: 78.57,
    social: 50.00
  },
  { 
    name: "Semen Indonesia (Persero) Tbk",
    sector: "Basic Materials",
    all: 56.58,
    anti_coruption: 65.63,
    environment: 85.71,
    social: 33.33
  },
  { 
    name: "PT Bank BRIsyariah Tbk / Bank Syariah Indonesia",
    sector: "Financials",
    all: 56.58,
    anti_coruption: 43.75,
    environment: 64.29,
    social: 66.67
  },
  { 
    name: "PT Bank Tabungan Negara (Persero) Tbk",
    sector: "Financials",
    all: 56.58,
    anti_coruption: 65.63,
    environment: 64.29,
    social: 43.33
  },
  { 
    name: "Kimia Farma Tbk",
    sector: "Healthcare",
    all: 56.58,
    anti_coruption: 68.75,
    environment: 64.29,
    social: 40.00
  },
  { 
    name: "Bank Pan Indonesia Tbk",
    sector: "Financials",
    all: 56.58,
    anti_coruption: 40.63,
    environment: 50.00,
    social: 76.67
  },
  { 
    name: "Samudera Indonesia Tbk",
    sector: "Transportation & Logistic",
    all: 55.26,
    anti_coruption: 50.00,
    environment: 85.71,
    social: 46.67
  },
  { 
    name: "PT Indosat Tbk",
    sector: "Infrastructures",
    all: 55.26,
    anti_coruption: 56.25,
    environment: 57.14,
    social: 53.33
  },
  { 
    name: "Bakrie & Brothers Tbk",
    sector: "Industrials",
    all: 55.26,
    anti_coruption: 65.63,
    environment: 50.00,
    social: 46.67
  },
  { 
    name: "Indofood Sukses Makmur Tbk",
    sector: "Consumer Non-Cyclicals",
    all: 53.95,
    anti_coruption: 43.75,
    environment: 64.29,
    social: 60.00
  },
  { 
    name: "Dayamitra Telekomunikasi Tbk",
    sector: "Infrastructures",
    all: 53.95,
    anti_coruption: 56.25,
    environment: 50.00,
    social: 53.33
  },
  { 
    name: "PT Indoritel Makmur Internasional Tbk.",
    sector: "Financials",
    all: 53.95,
    anti_coruption: 46.88,
    environment: 57.14,
    social: 60.00
  },
  { 
    name: "Sinarmas Multiartha Tbk",
    sector: "Financials",
    all: 53.95,
    anti_coruption: 65.63,
    environment: 42.86,
    social: 46.67
  },
  { 
    name: "Japfa Comfeed Indonesia Tbk",
    sector: "Consumer Non-Cyclicals",
    all: 52.63,
    anti_coruption: 43.75,
    environment: 71.43,
    social: 53.33
  },
  { 
    name: "Vale Indonesia Tbk",
    sector: "Basic Materials",
    all: 51.32,
    anti_coruption: 43.75,
    environment: 78.57,
    social: 46.67
  },
  { 
    name: "Bank Bukopin Tbk",
    sector: "Financials",
    all: 51.32,
    anti_coruption: 65.63,
    environment: 7.14,
    social: 56.67
  },
  { 
    name: "Energi Mega Persada Tbk",
    sector: "Energy",
    all: 51.32,
    anti_coruption: 37.50,
    environment: 57.14,
    social: 63.33
  },
  { 
    name: "Kalbe Farma Tbk",
    sector: "Healthcare",
    all: 51.32,
    anti_coruption: 53.13,
    environment: 42.86,
    social: 53.33
  },
  { 
    name: "Bumi Serpong Damai Tbk",
    sector: "Properties & Real Estate",
    all: 50.00,
    anti_coruption: 46.88,
    environment: 64.29,
    social: 46.67
  },
  { 
    name: "PT Waskita Karya (Persero) Tbk",
    sector: "Infrastructures",
    all: 50.00,
    anti_coruption: 50.00,
    environment: 57.14,
    social: 46.67
  },
  { 
    name: "PT Bank Negara Indonesia (Persero) Tbk",
    sector: "Financials",
    all: 48.68,
    anti_coruption: 37.50,
    environment: 71.43,
    social: 50.00
  },
  { 
    name: "Indofood CBP Sukses Makmur Tbk",
    sector: "Consumer Non-Cyclicals",
    all: 48.68,
    anti_coruption: 34.38,
    environment: 71.43,
    social: 53.33
  },
  { 
    name: "Mitra Adiperkasa Tbk",
    sector: "Consumer Cyclicals",
    all: 48.68,
    anti_coruption: 43.75,
    environment: 50.00,
    social: 53.33
  },
  { 
    name: "PT Bank BTPN Tbk",
    sector: "Financials",
    all: 47.37,
    anti_coruption: 53.13,
    environment: 57.14,
    social: 36.67
  },
  { 
    name: "Charoen Pokphand Indonesia Tbk",
    sector: "Consumer Non-Cyclicals",
    all: 47.37,
    anti_coruption: 37.50,
    environment: 50.00,
    social: 56.67
  },
  { 
    name: "Bukalapakcom Tbk",
    sector: "Technology",
    all: 46.05,
    anti_coruption: 59.38,
    environment: 57.14,
    social: 26.67
  },
  { 
    name: "Indomobil Multi Jasa Tbk",
    sector: "Financials",
    all: 40.79,
    anti_coruption: 37.50,
    environment: 35.71,
    social: 46.67
  },
  { 
    name: "Gudang Garam Tbk",
    sector: "Consumer Non-Cyclicals",
    all: 39.47,
    anti_coruption: 31.25,
    environment: 64.29,
    social: 36.67
  },
  { 
    name: "Media Nusantara Citra Tbk",
    sector: "Consumer Cyclicals",
    all: 39.47,
    anti_coruption: 31.25,
    environment: 28.57,
    social: 53.33
  },
  { 
    name: "Indomobil Sukses Internasional Tbk",
    sector: "Consumer Cyclicals",
    all: 34.21,
    anti_coruption: 34.38,
    environment: 21.43,
    social: 40.00
  },
  { 
    name: "Salim Ivomas Pratama Tbk",
    sector: "Consumer Non-Cyclicals",
    all: 31.58,
    anti_coruption: 21.88,
    environment: 42.86,
    social: 36.67
   }
]